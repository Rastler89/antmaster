<div class="max-w-7xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-zinc-400 to-zinc-600">Historial de Revisiones</h1>
            <p class="text-muted text-sm mt-1">Registro de todas las propuestas aceptadas y rechazadas.</p>
        </div>
    </div>

    <?php if (empty($revisiones)): ?>
        <div class="glass-card p-12 text-center">
            <p class="text-zinc-400">Aún no hay registros de revisiones procesadas.</p>
        </div>
    <?php else: ?>
                ...
                <table class="w-full text-left">
                    <thead class="bg-white/5 text-[10px] uppercase font-black tracking-widest text-zinc-500">
                        <tr>
                            ...
                            <th class="px-6 py-4">Especie / Propuesta</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        <?php foreach ($revisiones as $rev): ?>
                            <?php 
                                ...
                                $name = $isNew ? ($cambios['nombre'] ?? 'Nueva Especie') : $rev['especie_nombre'];
                            ?>
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold <?= $isNew ? 'text-emerald-400' : 'text-blue-400' ?>">
                                            <?= $isNew ? '[NUEVA] ' : '' ?><?= htmlspecialchars($name) ?>
                                        </span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <?php if ($rev['estado'] === 'aprobada'): ?>
                                        <span class="px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-500 text-[10px] uppercase font-black border border-emerald-500/20">Aprobada</span>
                                    <?php else: ?>
                                        <span class="px-2 py-1 rounded-md bg-red-500/10 text-red-500 text-[10px] uppercase font-black border border-red-500/20">Rechazada</span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4">
                                    <?php if ($rev['estado'] === 'rechazada' && !empty($rev['motivo_rechazo'])): ?>
                                        <div class="text-xs text-zinc-400 bg-red-500/5 p-2 rounded-lg border border-red-500/10 max-w-xs">
                                            <strong class="text-red-400/80">Motivo:</strong> <?= htmlspecialchars($rev['motivo_rechazo']) ?>
                                        </div>
                                    <?php elseif ($rev['estado'] === 'aprobada'): ?>
                                        <span class="text-[10px] text-zinc-500 italic">Cambios aplicados con éxito.</span>
                                    <?php else: ?>
