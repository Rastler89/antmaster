<div class="max-w-5xl mx-auto">
    ...
    <div class="mb-10">
        <h1 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">Personalización y Accesibilidad</h1>
        <p class="text-muted mt-2">Adapta tu experiencia en AntMaster Pro según tus preferencias y necesidades visuales.</p>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="glass-card p-6 space-y-6">
                    ...
                    <label class="flex items-center justify-between cursor-pointer group">
                        <span class="text-sm font-medium text-muted group-hover:text-main transition-colors">Alto Contraste</span>
                    ...
                    <!-- Reducir Movimiento -->
                    <label class="flex items-center justify-between cursor-pointer group">
                        <span class="text-sm font-medium text-muted group-hover:text-main transition-colors">Reducir Movimiento</span>
                        <div class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div class="pt-4 border-t border-white/5">
                        <label class="block text-sm font-medium text-muted mb-3">Modo Daltonismo</label>
                        <select name="colorblind_mode" class="magic-input text-xs">
                            <option value="none" <?= ($settings['colorblind_mode'] ?? 'none') === 'none' ? 'selected' : '' ?>>Ninguno (Estándar)</option>
                            <option value="protanopia" <?= ($settings['colorblind_mode'] ?? '') === 'protanopia' ? 'selected' : '' ?>>Protanopia (Rojo-Verde)</option>
                            <option value="deuteranopia" <?= ($settings['colorblind_mode'] ?? '') === 'deuteranopia' ? 'selected' : '' ?>>Deuteranopia (Verde-Rojo)</option>
                            <option value="tritanopia" <?= ($settings['colorblind_mode'] ?? '') === 'tritanopia' ? 'selected' : '' ?>>Tritanopia (Azul-Amarillo)</option>
                        </select>
                        <p class="mt-2 text-[10px] text-zinc-500 leading-tight">Aplica un filtro de matriz de color para compensar la percepción.</p>
                    </div>
                </div>
