...
<body class="antialiased selection:bg-blue-500/30 selection:text-white transition-colors duration-500 <?= $colorblindClass ?> <?= $reducedMotionClass ?>">
    ...
    <!-- Navigation -->
    <?php
        // Fetch global system alert
        require_once '../core/Database.php';
        $alertStmt = Database::getConnection()->prepare("SELECT setting_value FROM system_settings WHERE setting_key = 'system_alert_message' LIMIT 1");
        $alertStmt->execute();
        $systemAlert = $alertStmt->fetchColumn();
    ?>

    <?php if (!empty($systemAlert)): ?>
        <div class="bg-gradient-to-r from-red-600/90 to-orange-600/90 backdrop-blur-md text-white border-b border-white/10 relative z-[70] animate-fade-in">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center animate-pulse">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                    </div>
                    <span class="text-xs font-black uppercase tracking-[0.2em] italic"><?= htmlspecialchars($systemAlert) ?></span>
                </div>
                <button onclick="this.parentElement.parentElement.remove()" class="p-2 hover:bg-white/10 rounded-xl transition-all" title="Cerrar aviso">
