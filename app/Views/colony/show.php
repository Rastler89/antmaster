...
<div class="colony-profile-container animate-fade-in">
    ...
    <div class="relative h-64 md:h-[450px] rounded-[3.5rem] overflow-hidden mb-10 group shadow-2xl ring-1 ring-white/10">
        <?php if ($colony['imagen']): ?>
            <img src="<?= asset('uploads/colonies/' . $colony['imagen']) ?>" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
        <?php else: ?>
                                ...
                                <div class="glass-card p-6 sm:p-8 hover:border-blue-500/30 transition-all group">
                                    ...
                                    <?php if ($entry['imagen_url']): ?>
                                        <div class="rounded-3xl overflow-hidden border border-white/5 bg-zinc-900/50">
                                            <img src="<?= asset('uploads/diary/' . $entry['imagen_url']) ?>" class="w-full h-auto object-cover max-h-[500px] hover:scale-[1.02] transition-transform duration-700 cursor-zoom-in" onclick="openLightbox(this.src)">
                                        </div>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                    ...
                    <?php else: ?>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            <?php foreach ($media as $item): ?>
                                <div class="relative aspect-square rounded-2xl overflow-hidden border border-white/5 group bg-zinc-900 cursor-zoom-in" onclick="openLightbox('<?= $item['url'] ?>')">
